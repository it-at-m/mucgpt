...
from agent.tools.diagram_agent import DiagramAgent  # NEW IMPORT
...
# NEW: Diagram tool summary
DIAGRAM_SUMMARY = "Generates and iteratively refines Mermaid or Graphviz diagrams from a description."  # one-line
...
# NEW: Detailed instructions for diagram tool
DIAGRAM_DETAILED = textwrap.dedent(
    """
    **Diagram (Mermaid / Graphviz)**

    Description: Generates a Mermaid or Graphviz (DOT) diagram from a textual description, validates syntax (library + heuristics), critiques, and optionally refines up to a configurable number of revisions.

    Use for:
    • Architecture overviews
    • Process / flow visualization
    • Service dependency graphs
    • Sequence / class / state representations

    Parameters:
    - diagram_type (required) One of: mermaid, graphviz
    - description (required) Natural language description of the system / flow
    - max_revisions (optional, default 2) Maximum refinement cycles

    Output:
    - A code block wrapped in ```MUCGPTDiagram <type>``` containing ONLY the final diagram code (no prose). `<type>` is `mermaid` or `graphviz`.

    Best Practices:
    - Provide clear relationships (edges / arrows) and meaningful labels
    - Avoid excessive decorative attributes; prioritize clarity
    - For Mermaid: start with graph direction or specialized diagram keyword
    - For Graphviz: start with (di)graph declaration and ensure balanced braces
    - When refining: preserve correct parts; change only flagged issues
    - Reference previous diagram blocks: ```MUCGPTDiagram mermaid ...``` or ```MUCGPTDiagram graphviz ...```
    """
)
...
```MUCGPTDiagram mermaid
Mermaid diagram code only
...
```

```MUCGPTDiagram graphviz
Graphviz (DOT) diagram code only
...
```
...
def make_diagram_tool(model: RunnableSerializable, logger: logging.Logger = None):
    @tool(
        "Diagram",
    ...
    def diagram_tool(
        ...
        """Generate (and optionally refine) a diagram.

        diagram_type: mermaid | graphviz
        description: natural language specification
        max_revisions: number of refinement iterations (0..5 reasonable)
        """
        writer = get_stream_writer()
        writer(
            ToolStreamChunk(
                state=ToolStreamState.STARTED,
                content=f"Erzeuge {diagram_type} Diagramm...",
                tool_name="Diagram",
            ).model_dump_json()
        )
        # Instantiate agent per call so per-run max revisions can differ
        agent = DiagramAgent(
            model=model,
            logger=logger or getLogger("diagram-tool"),
            writer=writer,
        ...
        # Wrap output as specified
        wrapped = f"```MUCGPTDiagram {diagram_type.lower()}\n{code}\n```"
        writer(
            ToolStreamChunk(
                state=ToolStreamState.ENDED,
                content=f"Diagramm abgeschlossen ({diagram_type}).",
                tool_name="Diagram",
            ).model_dump_json()
        )
        return wrapped
...
class ToolCollection:
    ...
    def __init__(self, model: RunnableSerializable, logger: logging.Logger = None):
        ...
        self._diagram_tool = make_diagram_tool(self.model, self.logger)  # NEW
    ...
    @property
    def diagram(self):  # NEW
        return self._diagram_tool

    def get_tools(self):
        """Return a list of all tool callables."""
        return [self._brainstorm_tool, self._simplify_tool, self._diagram_tool]

    def get_all(self, enabled_tools: list[str] = None) -> list:
        """Return a list of tools, optionally filtered by enabled_tools."""
        all_tools = [self._brainstorm_tool, self._simplify_tool, self._diagram_tool]
        if enabled_tools:
    ...
    def list_tool_metadata(lang: str = "Deutsch") -> ToolListResponse:
        ...
        tool_metadata = {
            "deutsch": {
                ...
                "Diagram": {
                    "name": "Diagramm",
                    "description": "Erstellt und verbessert Mermaid- oder Graphviz-Diagramme aus einer Beschreibung.",
                },
            },
            "english": {
                "Brainstorming": {
                ...
                "Diagram": {
                    "name": "Diagram",
                    "description": "Generates and refines Mermaid or Graphviz diagrams from a description.",
                },
            },
            "français": {
                "Brainstorming": {
                ...
                "Diagram": {
                    "name": "Diagramme",
                    "description": "Génère et affine des diagrammes Mermaid ou Graphviz à partir d'une description.",
                },
            },
            "bairisch": {
                "Brainstorming": {
                ...
                "Diagram": {
                    "name": "Diagramm",
                    "description": "Erzeugt und verbessert Mermaid/Graphviz Diagramm aus Beschreibung.",
                },
            },
            "ukrainisch": {
                "Brainstorming": {
                ...
                "Diagram": {
                    "name": "Діаграма",
                    "description": "Генерує та покращує діаграми Mermaid або Graphviz з опису.",
                },
            },
        }
        ...
        diagram_tool = make_diagram_tool(DummyModel(), dummy_logger)

        # Build the list using actual tool names for lookup
        tools = []
        for tool_id in [brainstorm_tool, simplify_tool, diagram_tool]:
