...
class Repository:
    ...
    def addInfo(self, info: Requestinfo):
        info.department = self.truncate_string(
            info.department, DEPARTMENT_STRING_LENGTH
        )
    ...
    def countByDepartment(self):
        with Session(self.engine) as session:
            queryResult = session.query(
                Requestinfo.department, func.count(Requestinfo.tokencount)
            ).group_by(Requestinfo.department)
    ...
    def sumByDepartment(self):
        with Session(self.engine) as session:
            queryResult = session.query(
                Requestinfo.department, func.sum(Requestinfo.tokencount)
            ).group_by(Requestinfo.department)
            results = queryResult.all()
            results = [
                {"department": row[0], "tokencount": float(row[1])} for row in results
            ]
            return results
    ...
    def avgByDepartment(self):
        with Session(self.engine) as session:
            queryResult = session.query(
                Requestinfo.department, func.avg(Requestinfo.tokencount)
            ).group_by(Requestinfo.department)
            results = queryResult.all()
            results = [
                {"department": row[0], "tokencount": float(row[1])} for row in results
    ...
    def export(self):
        memfile = io.StringIO()
        outcsv = csv.writer(
            memfile, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        ...
        [
            outcsv.writerow(
                [
                    ...
                    for column in Requestinfo.__mapper__.columns
                ]
            )
            for curr in self.getAll()
        ]
