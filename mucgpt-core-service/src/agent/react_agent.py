...
class _ConfiguredLangChainAgentGraph:
    ...
    def __init__(
        ...
        logger.info("Initializing MUCGPT ReAct agent graph with tools: %s", [tool.name for tool in tools])
    ...
    @staticmethod
    def get_schema_from_tools(enabled_tools: list[BaseTool] | None) -> DefaultAgentState:
    ...
    def _prepare_run(
        ...
        state_schema = _ConfiguredLangChainAgentGraph.get_schema_from_tools(tools_to_use)
    ...
    async def astream(
        ...
        **kwargs,
        ...
        model, messages, tools_to_use, state_schema = self._prepare_run(input_data, config)
