...
from agent.tools.create_bpmn import create_bpmn
...
BPMN_SUMMARY = "Creates BPMN 2.0 diagrams from process descriptions."  # one-line
...
BPMN_DETAILED = textwrap.dedent(
    """
    **BPMN Erstellen (Create BPMN)**

    Description: Creates BPMN 2.0 diagrams from textual process descriptions.

    Use for:
    • Business process documentation
    • Workflow visualization
    • Process analysis and optimization
    • Converting narrative process descriptions to standardized diagrams

    Technical:
    • Extracts process steps, decision points, and flows using structured LLM output
    • Generates valid BPMN 2.0 XML with visual layout
    • Validates structure and semantics
    • Supports tasks, gateways (exclusive, parallel, inclusive), and events

    Parameter:
    - process_text (required) Full process description in natural language

    Best Practices:
    - Provide complete process descriptions with clear step sequences
    - Mention decision points explicitly
    - Include start and end conditions when known
    - Use clear language for conditional branches
    - Reference earlier output blocks shown as ```MUCGPTBPMN ...``` when refining.
    """
)
...
```MUCGPTBPMN
BPMN XML diagram created by the BPMN Creator tool
...
```
...
def make_bpmn_tool(model: RunnableSerializable, logger: logging.Logger = None):
    @tool(
        "BPMN_Erstellen",
        description=BPMN_SUMMARY,
    )
    def bpmn_tool(process_text: str):
        writer = get_stream_writer()
        writer(
            ToolStreamChunk(
                state=ToolStreamState.STARTED,
                content="Starte BPMN-Erstellung aus Prozessbeschreibung",
                tool_name="BPMN Creator",
            ).model_dump_json()
        )
        result = create_bpmn(process_text, model, logger, writer=writer)
        return result
...
class ToolCollection:
    ...
    def __init__(self, model: RunnableSerializable, logger: logging.Logger = None):
        ...
        self._bpmn_tool = make_bpmn_tool(self.model, self.logger)
    ...
    @property
    def bpmn(self):
        return self._bpmn_tool

    def get_tools(self):
        """Return a list of all tool callables."""
        return [self._brainstorm_tool, self._simplify_tool, self._bpmn_tool]

    def get_all(self, enabled_tools: list[str] = None) -> list:
        """Return a list of tools, optionally filtered by enabled_tools."""
        all_tools = [self._brainstorm_tool, self._simplify_tool, self._bpmn_tool]
        if enabled_tools:
    ...
    def list_tool_metadata(lang: str = "Deutsch") -> ToolListResponse:
        ...
        tool_metadata = {
            "deutsch": {
                ...
                "BPMN_Erstellen": {
                    "name": "BPMN Erstellen",
                    "description": "Erstellt BPMN 2.0 Diagramme aus Prozessbeschreibungen in natürlicher Sprache.",
                },
            },
            "english": {
                "Brainstorming": {
                ...
                "BPMN_Erstellen": {
                    "name": "Create BPMN",
                    "description": "Creates BPMN 2.0 diagrams from natural language process descriptions.",
                },
            },
            "français": {
                "Brainstorming": {
                ...
                "BPMN_Erstellen": {
                    "name": "Créer BPMN",
                    "description": "Crée des diagrammes BPMN 2.0 à partir de descriptions de processus en langage naturel.",
                },
            },
            "bairisch": {
                "Brainstorming": {
                ...
                "BPMN_Erstellen": {
                    "name": "BPMN erstöin",
                    "description": "Macht BPMN 2.0 Diagramme aus Prozessbeschreibungen in normaler Sprach.",
                },
            },
            "ukrainisch": {
                "Brainstorming": {
                ...
                "BPMN_Erstellen": {
                    "name": "Створити BPMN",
                    "description": "Створює діаграми BPMN 2.0 з описів процесів природною мовою.",
                },
            },
        }
        ...
        bpmn_tool = make_bpmn_tool(DummyModel(), dummy_logger)

        # Build the list using actual tool names for lookup
        tools = []
        for tool_id in [brainstorm_tool, simplify_tool, bpmn_tool]:
