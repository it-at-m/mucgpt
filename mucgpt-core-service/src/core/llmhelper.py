...
def getModel(
    ...
    if default_model.type == "AZURE":
        ...
        raise ModelsConfigurationException(
            f"Unknown model type: {default_model.type}. Currently only `AZURE` and `OPENAI` are supported."
        )
    ...
    llm = llm.configurable_fields(
        ...
        streaming=ConfigurableField(
            id="llm_streaming", name="Streaming", description="Should the LLM Stream"
        ),
        callbacks=ConfigurableField(
            id="llm_callbacks", name="Callbacks", description="Callbacks for the llm"
        ),
    ).configurable_alternatives(
        ConfigurableField(id="llm"), default_key=models[0].llm_name, **alternatives
    )
    return llm
